...
\begin{definition}
    Let $f: V \to W$ be a linear map. The kernel of $f$ is the set of all vectors $v \in V$ such that $f(v) = 0$.
\end{definition}
\begin{definition}
    Let $f: V \to W$ be a linear map. The image of $f$ is the set of all vectors $w \in W$ such that $f(v) = w$ for some $v \in V$.
\end{definition}
\begin{definition}
    Let $f: V \to W$ be a linear map. The cokernel of $f$ is the quotient space $W / \operatorname{Im} f$.
\end{definition}

Let $f:M\to N$ ba a morphism from $M$ to $N$ where $M,N \in \operatorname{Rep}Q$. $M=(V_i, \varphi_\alpha)$ and $N=(U_i, \psi_\alpha)$. Define $\operatorname{ker}f \in \operatorname{Rep}Q$ as $(L_i, \chi_\alpha)$ where $L_i=\operatorname{ker}f_i$ and $\chi_\alpha=\varphi_\alpha|_{L_i}$. $\forall v_i \in L_i$, $\chi_\alpha(v_i)=\varphi_\alpha(v_i)$.

We want to show that $\operatorname{ker}f$ is a representation in $\operatorname{Rep}Q$. i.e. $\chi_{\alpha}: L_i \to L_j$ for $\alpha: i \to j$ 

In fact, $ \forall v_i \in L_i $, $ f_i(v_i)=0 $, then $ f_j (\chi_\alpha(v_i)) = f_j (\varphi_\alpha(v_i)) = \psi_\alpha(f_i(v_i)) = \psi_\alpha(0) = 0 $. Hence $ \chi_\alpha(v_i) \in \operatorname{ker}f_j = L_j $. Hence $ \chi_\alpha: L_i \to L_j $ for $\alpha: i \to j$.

The inclusion $ L_i \hookrightarrow V_i $ defines an injective morphism from $\operatorname{ker}f$ to $M$ as the following diagram commutes:

\[
\begin{tikzcd}
L_i \arrow[r, "\chi_\alpha"] \arrow[d, "incl."] & L_j \arrow[d, "incl."] \\
V_i \arrow[r, "\varphi_\alpha"] & V_j
\end{tikzcd}
\]

Define $ \operatorname{coker}f \in \operatorname{Rep}Q $ as $ (R_i, \nu_\alpha) $ where $ R_i = N_i / \operatorname{Im}f_i $ and $ \nu_\alpha: R_i \to R_j $ for $\alpha: i \to j$ is defined as $ \nu_\alpha(n_i + f_i(V_i)) = \psi_\alpha(n_i) + f_j(V_j) $.

We need to show that all $\nu_\alpha$ are well-defined.

Let $ n_i + f_i(V_i) = n_i' + f_i(V_i) $, then $ n_i - n_i' \in f_i(V_i) $. That is, $ \exists v_i \in V_i $ such that $ n_i - n_i' = f_i(v_i) $. Then
\[
\psi_\alpha(n_i - n_i') = \psi_\alpha(f_i(v_i)) = f_j(\varphi_\alpha(v_i)) \in f_j(V_j)
\]
Hence, $ \psi_\alpha(n_i) - \psi_\alpha(n_i') \in f_j(V_j) $. That is, $ \nu_\alpha(n_i + f_i(V_i)) = \psi_\alpha(n_i) + f_j(V_j) $ is well-defined.

The projection $ N \to \operatorname{coker}f $ is a surjective morphism in $\operatorname{Rep}Q$ as the following diagram commutes:
\[
\begin{tikzcd}
N_i \arrow[r, "\psi_\alpha"] \arrow[d, "proj."] & N_j \arrow[d, "proj."] \\
R_i \arrow[r, "\nu_\alpha"] & R_j 
\end{tikzcd}
\]
...
\begin{definition} 
    Recall that for a morophism $ f: M \to N $, we have a representation 
    \[
    \begin{tikzcd}
...
First two representations are irriducible and indecomposable, the last one is reducible but indecomposable. 
...
\]
Hence the diagram commutes for every arrow $\alpha$, so $\overline{f}:M/\operatorname{ker}f\to \operatorname{im}f$ is a morphism in $\operatorname{Rep}Q$. Since each $\overline{f}_i$ is an isomorphism, $\overline{f}$ is an isomorphism of representations. Therefore
\[
...
\end{bmatrix}
\]
Let $ m_i \in \operatorname{ker}g_i $, then $ 0=\varphi_\alpha''(g_i(m_i))=g_j \varphi_\alpha  (m_i) $ where $ \varphi_\alpha (m_i) \in \operatorname{ker}g_j $   
\end{proof}
...
If $ X \in \operatorname{Rep}Q $ then $ \operatorname{Hom}_{\mathcal{C}}(X,-) $ is a covariant functor from $ \operatorname{Rep}Q $ to the category of $ k $-vector spaces.
