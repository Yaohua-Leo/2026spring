...
\begin{theorem}
    Let ${}_SU_{R}$ be a left-$S$-right-$R$-bimodule. Then the functor
    \[
    U\otimes_R - \;:\; R\text{-Mod}\longrightarrow S\text{-Mod}
    \]
    is right exact.
...
\begin{proof}
    ...
    \]
    be an exact sequence of left $R$-modules. We must show that
    \[
    U\otimes_R M' \xrightarrow{\;1_U\otimes f\;} U\otimes_R M
    \xrightarrow{\;1_U\otimes g\;} U\otimes_R M'' \to 0
    \]
    is exact in $S\text{-}\mathrm{Mod}$.
    
    First, $1_U\otimes g$ is surjective. Indeed, every element of $U\otimes_R M''$ is a
    finite sum of elementary tensors of the form $u\otimes m''$, with $u\in U$ and
    $m''\in M''$. Since $g$ is surjective, for each $m''\in M''$ there exists $m\in M$
    such that $g(m)=m''$. Hence
    \[
    u\otimes m'' = u\otimes g(m) = (1_U\otimes g)(u\otimes m),
    \]
    so $1_U\otimes g$ is surjective.
    
    Next, since $g\circ f=0$, we have
    \[
    (1_U\otimes g)\circ (1_U\otimes f)=1_U\otimes (g\circ f)=0.
    \]
    Therefore,
    \[
    \operatorname{im}(1_U\otimes f)\subseteq \ker(1_U\otimes g).
    \]
    
    It remains to prove the reverse inclusion. Consider the quotient module
    \[
    (U\otimes_R M)/\operatorname{im}(1_U\otimes f).
    \]
    Define a map
    \[
    \Phi : U\times M'' \longrightarrow (U\otimes_R M)/\operatorname{im}(1_U\otimes f)
    \]
    by
    \[
    \Phi(u,m'')=\overline{u\otimes m},
    \]
    where $m\in M$ is any element such that $g(m)=m''$.
    
    We first check that $\Phi$ is well defined. Suppose $m_1,m_2\in M$ both satisfy
    $g(m_1)=g(m_2)=m''$. Then
    \[
    m_1-m_2\in \ker g=\operatorname{im} f,
    \]
    so there exists $m'\in M'$ such that $m_1-m_2=f(m')$. Hence
    \[
    u\otimes m_1-u\otimes m_2
    = u\otimes (m_1-m_2)
    = u\otimes f(m')
    = (1_U\otimes f)(u\otimes m'),
    \]
    which lies in $\operatorname{im}(1_U\otimes f)$. Thus
    \[
    \overline{u\otimes m_1}=\overline{u\otimes m_2},
    \]
    so $\Phi$ is well defined.
    
    It is straightforward to verify that $\Phi$ is $R$-balanced, hence by the universal
    property of the tensor product there exists a unique $S$-module homomorphism
    \[
    \overline{\Phi}: U\otimes_R M'' \longrightarrow
    (U\otimes_R M)/\operatorname{im}(1_U\otimes f)
    \]
    such that
    \[
    \overline{\Phi}(u\otimes m'')=\overline{u\otimes m},
    \qquad g(m)=m''.
    \]
    
    Now define
    \[
    \Psi : U\otimes_R M \longrightarrow U\otimes_R M'',\qquad
    \Psi(u\otimes m)=u\otimes g(m).
    \]
    This is exactly $1_U\otimes g$. Since
    \[
    \operatorname{im}(1_U\otimes f)\subseteq \ker(1_U\otimes g),
    \]
    the map $\Psi$ factors uniquely through the quotient, giving an $S$-module homomorphism
    \[
    \widetilde{\Psi} :
    (U\otimes_R M)/\operatorname{im}(1_U\otimes f)\longrightarrow U\otimes_R M''.
    \]
    
    We now show that $\overline{\Phi}$ and $\widetilde{\Psi}$ are inverse to each other.
    For $u\otimes m''\in U\otimes_R M''$, choose $m\in M$ with $g(m)=m''$. Then
    \[
    (\widetilde{\Psi}\circ \overline{\Phi})(u\otimes m'')
    =
    \widetilde{\Psi}(\overline{u\otimes m})
    =
    u\otimes g(m)
    =
    u\otimes m''.
    \]
    Conversely, for $\overline{u\otimes m}\in (U\otimes_R M)/\operatorname{im}(1_U\otimes f)$,
    \[
    (\overline{\Phi}\circ \widetilde{\Psi})(\overline{u\otimes m})
    =
    \overline{\Phi}(u\otimes g(m))
    =
    \overline{u\otimes m}.
    \]
    Thus $\overline{\Phi}$ is an isomorphism.
    
    Therefore,
    \[
    (U\otimes_R M)/\operatorname{im}(1_U\otimes f)\cong U\otimes_R M'',
    \]
    and hence
    \[
    \ker(1_U\otimes g)=\operatorname{im}(1_U\otimes f).
    \]
    So the sequence
    \[
    U\otimes_R M' \xrightarrow{\;1_U\otimes f\;} U\otimes_R M
    \xrightarrow{\;1_U\otimes g\;} U\otimes_R M'' \to 0
    \]
    is exact. This proves that the functor $U\otimes_R -$ is right exact.
    \end{proof}
...
\begin{theorem}
    ...
    \begin{enumerate}
        ...
        \item For every epimorphism $f:M\to N$ of left $R$-modules, the induced map
              ...
              \]
              is surjective.
        \item For every ring $S$ and every right $S$-module structure on $P$ making ${}_R P_S$
              an $R$-$S$-bimodule, the functor
              \[
                  \Hom_R(P,-): {}_R\mathrm{Mod}\longrightarrow \mathrm{Mod}\text{-}S
              \]
              is exact.
...
\begin{proof}
    ...
    Let $f:M\to N$ be an epimorphism. A morphism $\gamma:P\to N$ admits a lift
    $\widetilde{\gamma}:P\to M$ with $f\widetilde{\gamma}=\gamma$ if and only if $\gamma$ lies
    in the image of
    \[
    ...
    \textbf{(3) $\Longrightarrow$ (4).}
    This is immediate by taking the codomain category to be $\mathbf{Ab}$.

    \textbf{(4) $\Longrightarrow$ (2).}
    Apply (4) to the short exact sequence
    \[
        0\longrightarrow \ker f \longrightarrow M \xrightarrow{\,f\,} N \longrightarrow 0.
    \]
    Then $\Hom_R(P,f)$ is surjective.

    \textbf{(2) $\Longrightarrow$ (3).}
    By the left exactness of $\Hom_R(P,-)$, it remains only to prove surjectivity at the last
    term in every short exact sequence. But this is exactly the content of (2).
\end{proof}
...
\begin{corollary}
    A left $R$-module $P$ is finitely generated and projective if and only if for some
    $R$-module $P'$ and some integer $n\ge 0$ there is an $R$-isomorphism
    \[
        P\oplus P'\cong R^n.
    \]
\end{corollary}
...
\begin{proof}
    ...
    \[
        Y:=\{(p,p')\in P\oplus P' \mid \lambda(p)=\lambda'(p')\}.
    \]
    ...
    \[
        ...
        \mu(p,p')=p.
    \]
    Since $P$ and $P'$ are projective, both sequences split. Therefore
    \[
        Y\cong K\oplus P'
        \qquad\text{and}\qquad
        Y\cong K'\oplus P.
...
\begin{lemma}
    Let $R$ be a commutative ring with $1$, and let $X$, $Y$, $M$ be $R$-modules. Then there is
    ...
    \]
    Its image consists exactly of those morphisms from $X$ to $Y$ which factor through some
    finite direct power $M^n$ of $M$.
\end{lemma}

\begin{proof}
    The map is well defined and $R$-bilinear because $R$ is commutative, so it induces an
    ...
    Define
    \[
        \alpha:X\to M^n,\qquad \alpha(x)=(\varphi_1(x),\dots,\varphi_n(x)),
    \]
    and
    \[
        \beta:M^n\to Y,\qquad \beta(m_1,\dots,m_n)=\sum_{i=1}^n \psi_i(m_i).
    \]
    Then
    \[
        f=\beta\circ \alpha,
    \]
    so every element of the image factors through $M^n$ for some $n$.

    Conversely, suppose
    \[
        f:X\xrightarrow{\,u\,} M^n \xrightarrow{\,v\,} Y
    \]
    factors through $M^n$. Let $\pi_i:M^n\to M$ be the $i$th projection and
    $\iota_i:M\to M^n$ the $i$th inclusion. Then
    \[
        \operatorname{id}_{M^n}=\sum_{i=1}^n \iota_i\pi_i,
    \]
    hence
    \[
        f=v\circ u
        =v\circ \Bigl(\sum_{i=1}^n \iota_i\pi_i\Bigr)\circ u
        =\sum_{i=1}^n (v\circ \iota_i)\circ (\pi_i\circ u).
    \]
    Therefore
    \[
        f=\Theta_M^{X,Y}\Bigl(\sum_{i=1}^n (\pi_i\circ u)\otimes (v\circ \iota_i)\Bigr),
    \]
    and so $f$ belongs to the image.
\end{proof}
...
\begin{proof}
    ...
    Since finite free modules are finitely generated projective, every morphism in
    $\operatorname{Im}(\tau_{X,Y})$ is projective.
...
\begin{proof}
    Let $f\in \Hom_R^{\operatorname{pr}}(M,M)$. Then $f$ factors through a finitely generated
    projective module $P$:
    \[
        M\xrightarrow{\,u\,} P \xrightarrow{\,v\,} M.
    \]
    For any $a,b\in \operatorname{End}_R(M)$,
    \[
        a\circ f\circ b=(a\circ v)\circ u\circ b
    \]
    still factors through $P$. Hence $a\circ f$ and $f\circ b$ are projective morphisms, so
    $\Hom_R^{\operatorname{pr}}(M,M)$ is a two-sided ideal of $\operatorname{End}_R(M)$.
\end{proof}
...
\begin{proof}
    ...
    \textbf{(2) $\Longrightarrow$ (3).}
    By (2), $\operatorname{id}_P$ is projective, so there exist $\varphi_1,\dots,\varphi_n\in P^*$
    and $p_1,\dots,p_n\in P$ such that
    \[
...
\begin{proof}
    Suppose first that no proper submodule of $U$ maps onto $V$. Let $g:W\to U$ satisfy
    $f\circ g$ epimorphic. Then
    \[
        f(g(W))=V.
    \]
    ...
    Conversely, assume that whenever $f\circ g$ is epi, then $g$ is epi. Let $U_0\subsetneq U$
    be a proper submodule and let
    \[
...
\begin{proof}
    ...
    Choose $L_1\in \mathcal{S}$. Since $L_1$ is not maximal, there exists $L_2\in \mathcal{S}$
    with
    \[
        L_1\subsetneq L_2.
    \]
...
\begin{proposition}
    ...
    \]
    be a short exact sequence of $R$-modules. Then $M$ is Noetherian if and only if both $M'$
    and $M''$ are Noetherian.
\end{proposition}

\begin{proof}
    If $M$ is Noetherian, then every submodule of $M$ is finitely generated. Hence the submodule
    $M'$ is Noetherian. Also every submodule of $M''$ is the image of a submodule of $M$, so
    every submodule of $M''$ is finitely generated. Thus $M''$ is Noetherian.

    Conversely, assume that $M'$ and $M''$ are Noetherian. We use the maximal condition.
    Let $\pi:M\to M''$ be the quotient map and let $\mathcal{S}$ be a non-empty set of
    submodules of $M$. The set
    \[
        \pi(\mathcal{S}):=\{\pi(N)\mid N\in \mathcal{S}\}
    \]
    is a non-empty set of submodules of $M''$, so it has a maximal element. Choose $N\in
    \mathcal{S}$ such that $\pi(N)$ is maximal in $\pi(\mathcal{S})$.

    Now consider the set
    \[
        \mathcal{S}_N:=\{L\in \mathcal{S}\mid \pi(L)=\pi(N)\}.
    \]
    Then
    \[
        \{L\cap M' \mid L\in \mathcal{S}_N\}
    \]
    is a non-empty set of submodules of $M'$, hence has a maximal element. Replacing $N$ if
    necessary, we may assume that $N\cap M'$ is maximal among all $L\cap M'$ with $L\in
    \mathcal{S}_N$.

    We claim that $N$ is maximal in $\mathcal{S}$. Let $N\subseteq L$ with $L\in \mathcal{S}$.
    Then
    \[
        \pi(N)\subseteq \pi(L).
    \]
    By the maximality of $\pi(N)$, we get $\pi(L)=\pi(N)$, so $L\in \mathcal{S}_N$. Hence
    \[
        N\cap M' \subseteq L\cap M'.
    \]
    By the maximality of $N\cap M'$, it follows that
    \[
        L\cap M'=N\cap M'.
    \]
    For any $x\in L$, since $\pi(x)\in \pi(L)=\pi(N)$, there exists $y\in N$ such that
    \[
        \pi(x)=\pi(y).
    \]
    Then
    \[
        x-y\in \ker\pi=M'\cap L=N\cap M',
    \]
    so $x\in N$. Thus $L=N$, proving that $N$ is maximal. Therefore $M$ is Noetherian.
\end{proof}
...
\begin{lemma}
    An $R$-module $M$ is Artinian if and only if every non-empty set of submodules of $M$
...
\begin{remark}
    If an $R$-module $M$ is Noetherian (respectively Artinian), then we also say that $M$
    satisfies the ascending chain condition (respectively descending chain condition) on
...
\begin{proposition}
    ...
    \[
        0\longrightarrow M' \longrightarrow M \longrightarrow M'' \longrightarrow 0
    \]
    be a short exact sequence of $R$-modules. Then $M$ is Artinian if and only if both $M'$ and
    $M''$ are Artinian.
\end{proposition}
...
\begin{exercise}
    ...
    Apply the defining property of injectivity to the monomorphism $\varphi$ and the map
    $\operatorname{id}_J\colon J\to J$.
    ...
    \begin{exercise}
    Suppose that $M_i$ $(i\in I)$ are $R$-modules. Prove that
    \[
    ...
    We claim that $K_m=0$.
    Let $x\in K_m$.
    Because $f$ is surjective, there exists $y\in V$ such that
    \[
    f(y)=x.
    \]
    Now
    \[
    f^{m+1}(y)=f^m(f(y))=f^m(x)=0,
    \]
    so $y\in K_{m+1}=K_m$.
    Therefore
    \[
