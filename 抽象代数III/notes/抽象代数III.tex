...
\begin{remark}
If $R$ is commutative, then the left $R$-module and right $R$-module coincide.
\end{remark}

Consider $ M $ a $ R $-module, then $ \forall r \in R $, $ \varphi_r: M \rightarrow M $ by $ \varphi_r(m) = rm $ is a abelian group homomorphism. Let
\[
...
(7) Let $ R $ be a ringg with 1 and $ A \in M_n(R) $. The solution of $ AX=0 $ where $ X \in R^n $ is a righ t $ R $-module.   
\end{example}
...
\begin{example} 
(1) Let $ V $ be a vaector space over a field $ K $, then any subspace of $ V $ is a $ k $-submodule of $ V $.
...
(4) Let $ G $ be a group, which is a $ \operatorname{End}(G) $-module. Then any full invariant subgroup of $ G $ is a $ \operatorname{End}(G) $-submodule of $ G $.
\end{example}
...
\begin{definition}
    Let $R$ be a ring, and let $M$ and $N$ be left $R$-modules. A map
    \[
    f:M\to N
    \]
...
\begin{theorem} 
If $ f: M\to N $ is a $ R $-module homomorphism, then we have a canonical $ R $-module isomorphism:
\[
...
\begin{definition}
Direct sum \& direct product $\left(\bigoplus_{i \in I} M_i \; \& \; \prod_{i \in I} M_i\right)$.
