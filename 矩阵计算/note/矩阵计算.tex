...
\begin{titlepage}
    ...
    \HRule{1.5pt} \\[0.5cm]
    { \huge \bfseries Matrix Computation }\\[0.4cm] 
    \HRule{1.5pt} \\[1.5cm]
    ...
    \begin{spacing}{1.8}
        ...
        {\Large Prof. \textsc{Shangzhi Zeng}} \\[0.8cm]
...
\begin{tcolorbox}[
    ...
    This course establishes the mathematical foundation for matrix analysis and numerical computations, serving as a cornerstone for modern applications in machine learning, artificial intelligence, computer vision, and data science.
    
    The core curriculum addresses two fundamental classes of problems:
    \begin{enumerate}
        \item \textbf{Solving Linear Systems} ($Ax=b$):
        \begin{itemize}
            \item \textit{Square Systems} ($A \in \mathbb{R}^{n \times n}$): Solved via direct methods.
            \item \textit{Overdetermined Systems} ($A \in \mathbb{R}^{m \times n}, m > n$): Addressed using least squares or minimum norm solutions.
            \item \textit{Underdetermined Systems} ($m < n$): Explored through sparse solutions.
        \end{itemize}
        
        \item \textbf{Eigenvalue Problems} ($Av = \lambda v$):
        \begin{itemize}
            \item We investigate numerical algorithms for spectral analysis, noting that algebraic solutions for characteristic polynomials of degree $\ge 5$ are generally unavailable.
        \end{itemize}
    \end{enumerate}

    A critical distinction is drawn between \textbf{direct methods} and \textbf{iterative methods}. While direct methods provide exact solutions, their computational cost can be prohibitive for large-scale matrices. Consequently, we emphasize efficient iterative techniques---such as the Conjugate Gradient method and Krylov subspace methods (Lanczos, GMRES)---which are pivotal for high-dimensional computation.
\end{tcolorbox}

\begin{itemize}
    \item \textbf{Course Scope:}
    \begin{itemize}
        \item Basic matrix concepts, subspaces, and norms.
        \item Linear least squares and pseudo-inverses.
        \item Eigenvalue and Singular Value Decompositions (EVD, SVD); positive semidefinite matrices.
        \item Linear system solvers: LU, Cholesky, QR, and Schur decompositions.
        \item Krylov subspace methods (Lanczos, GMRES, etc.).
    \end{itemize}
\end{itemize}
