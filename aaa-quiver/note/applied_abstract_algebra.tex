...
\textbf{\textcolor{TopicColor}{Office hours:}} Mon: 12:30-14:30.
...
\begin{itemize}[leftmargin=*, nosep, label=\textcolor{TopicColor}{$\bullet$}]
    ...
    \item \textit{Alexander Kirillov Jr. - Quiver Representations and Quiver Varieties (2016, American Mathematical Society)}.
...
\begin{example}
    ...
    N:\quad
    \begin{tikzcd}[column sep=large]
    k^2 \arrow[r, "{\begin{bmatrix}0\\1\end{bmatrix}}"] &
    k^2 &
...
\begin{proof}
    ...
    \]
    is a split epimorphism. Therefore \(M_i\) is isomorphic to a direct summand of \(N_j\). Since \(N_j\) is indecomposable, we conclude that
    \[
    M_i \cong N_j.
    \]
...
\medskip\noindent\textbf{Kernel in \(\operatorname{Rep}Q\).}
...
\begin{lemma}
    With the notation above, \(\operatorname{ker}f\) is a representation of \(Q\), i.e.\ for each arrow \(\alpha: i \to j\) we have \(\chi_\alpha: L_i \to L_j\).
\end{lemma}
\begin{proof}
    ...
    \]
    Hence \(\chi_\alpha(v_i) \in \operatorname{ker}f_j = L_j\), so \(\chi_\alpha: L_i \to L_j\).
\end{proof}
...
\medskip\noindent\textbf{Cokernel in \(\operatorname{Rep}Q\).}
...
\begin{proof}
    ...
    \]
    Hence \(\psi_\alpha(n_i) - \psi_\alpha(n_i') \in f_j(V_j)\), so \(\psi_\alpha(n_i) + f_j(V_j) = \psi_\alpha(n_i') + f_j(V_j)\). Thus \(\nu_\alpha\) is well-defined. The commutativity of the representation maps for \(\operatorname{coker}f\) follows from that of \(N\).
\end{proof}
...
\begin{definition}[Cokernel of a morphism]
    ...
    For each arrow $\alpha:i\to j$, define
    \[
    \chi_\alpha:H_i\to H_j,\qquad
    \chi_\alpha\bigl(n_i+f_i(M_i)\bigr)
    :=\psi_\alpha(n_i)+f_j(M_j).
    \]
    Then
    \[
    \operatorname{coker}f:=(H_i,\chi_\alpha).
    \]
...
\begin{proof}
    ...
    We first check that $\chi_\alpha$ is well-defined. Suppose
    \[
    ...
    \[
    \psi_\alpha(f_i(m_i))
    =f_j(\varphi_\alpha(m_i))\in f_j(M_j).
    \]
    So
    \[
    \psi_\alpha(n_i)+f_j(M_j)=\psi_\alpha(n_i')+f_j(M_j),
    \]
    and thus $\chi_\alpha$ is well-defined.
...
\begin{example}
    ...
    In the second short exact sequence the middle term is the \emph{direct sum} $S_1 \oplus S_2$, with $f = (0,1)$ the inclusion of the second summand and $g = (1,0)$ the projection onto the first. Take the projection $h: S_1 \oplus S_2 \to S_2$ onto the second factor (at each vertex). Then $(hf)(s_2) = h(0,s_2) = s_2$ for all $s_2 \in S_2$, so $hf = 1_{S_2}$. Thus $f$ has a retraction and the sequence splits. Equivalently, the inclusion $h': S_1 \to S_1 \oplus S_2$ of the first summand satisfies $gh' = 1_{S_1}$, so $g$ has a section.

    By contrast, in the first sequence the middle term is $M$ (the representation $k \xrightarrow{1} k$), which is indecomposable and not isomorphic to $S_1 \oplus S_2$. There is no morphism from $M$ to $S_2$ giving a retraction of $f$, so the first sequence does not split.
\end{example}
...
&=\varphi_\alpha (m_i)-f_j(\psi_\alpha' (h_i(m_i)))\\
...
The only thing need to show now is that $ gh'=1_N $. Let $ n_i \in N_i $, then $ \exists m_i \in M_i $ such that $ n_i =g(m_i) $. Then $ g_ih_i'(n_i)=g_i(m-fh(m))=g(m)-g(fh(m))=g_i(m_i)=n_i $. Hence $ gh'=1_N $.

$ (\Leftarrow )$: 
Suppose $ g $ is a retraction. $ \exists h': N\to M $ such that $ gh'=1_N $. We will define $ h: M\to L $ and show that $ hf=1_L $. 

For any $ m $ we have $ g(m-h'g(m))=g(m)-g(h'g(m))=g(m)-g(m)=0 $. Hence $ m-h'g(m) \in \operatorname{ker}g = \operatorname{im}f $. Hence $ \exists ! l \in L $ such that $ f(l)=m-h'g(m)  $ since $ f $ is injective. Define $ h(m)=l $. Then 
...
\[
    \begin{tikzcd}
        ...
        \end{tikzcd}
\]
Take $ m_i \in M_i $, and for $ m_i $ we choose $ l_i \in L_i $ such that $ f(l_i)=m_i-h_i'g_i(m_i) $ and $ h_i(m_i)=l_i $ . Then we have
\[
\varphi_\alpha (h_i(m_i))=\varphi_\alpha' (l_i)
\]
On the other hand,
\begin{align*}
h_j\varphi_\alpha(m_i) & =h_j\varphi_\alpha(f_i(l_i)+h_i'g_i(m_i))\\
&= 
\end{align*}
\end{proof}
...
\]
is an element of $\operatorname{im}h_i'$. This shows that the lower left block of the matrix is zero and hence $\varpi_\alpha$ is a block diagonal matrix.
\end{proof}
...
\begin{corollary}
    ...
    \end{tikzcd}
    \]
    is split exact. By Theorem 1.10, it is enough to show that
    \[
...
representations, the irreducible morphisms between them, and the basic almost
split sequences.
...
\subsection*{Example 1: the quiver $1 \to 2$}
...
\subsection*{A convenient shorthand notation}
...
\[
\begin{tikzcd}[row sep=2.5em, column sep=3em]
& \frac{1}{2} \arrow[dl] \arrow[dr] & \\
2 & & 1
\end{tikzcd}
\]

\subsection*{Example 2: the quiver $1 \to 2 \leftarrow 3$}
...
& & 3 \\
& \frac{3}{2} \arrow[ur] & \\
2 \arrow[ur] \arrow[dr] & & \frac{13}{2} \arrow[uu] \arrow[dd] \\
& \frac{1}{2} \arrow[dr] & \\
& & 1
\end{tikzcd}
\]

\subsection*{Further non-split short exact sequences}
