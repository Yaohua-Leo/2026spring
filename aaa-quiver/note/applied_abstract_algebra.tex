...
\begin{definition}
    For each $i \in Q_0$, we define the representation $S(i)=(S(i)_i, \phi_\alpha)$ as the following:
    \begin{itemize}
\item $S(i)_i$
\item $S(i)_j=0$ for $j \neq i$
\item $\forall \alpha \in Q_1$, $\varphi_\alpha=0$
    \end{itemize}
\end{definition}

\begin{definition}
    For each $i \in Q_0$, we define the representation $P(i)=(P(i)_i, \psi_\alpha)$ as the following:
    \begin{itemize}
\item $P(i)_i=k$, corresponding to lazy path $(i|i)$ in vertex $i$
\item $P(i)_j$ is a vector space spanned by all paths from $i$ to $j$. In other words, all elements of $P(i)_j$ are of the form $\sum\limits_{c \in \mathcal{P}(i,j)} \lambda_c c$, where $\lambda_c \in k$ and $c$ denotes a path from $i$ to $j$.
\item Let $\alpha \in Q_1$ be an arrow from $j$ to $l$. Then $\psi_\alpha$ is the linear map from $P(i)_j$ to $P(i)_l$ defined by $\psi_\alpha(c: i \to j) = c' : i \to l$. Or we can write $\psi(c)=c' = \alpha c= (i | \alpha_1, \ldots, \alpha_k, \alpha | l)$ where $c=(i | \alpha_1, \ldots, \alpha_k | j)$
    \end{itemize}
\end{definition}
