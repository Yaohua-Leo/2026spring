...
\textbf{\textcolor{TopicColor}{Main book:}} \textit{Rqalf Schiffler, Quiver Representations}

\textbf{\textcolor{TopicColor}{References:}}
\begin{itemize}[leftmargin=*, nosep, label=\textcolor{TopicColor}{$\bullet$}]
    \item \textit{Assem, Simson, Skowroński, Elements of the Representation Theory of Associative Algebras, Vol. 1: Techniques of Representation Theory}, Cambridge University Press.
...
\subsection{Kernel in \(\operatorname{Rep}Q\).}
...
\subsection{Cokernel in \(\operatorname{Rep}Q\).}
...
Since $ g(m)-g(m') = g(m-m')=0 $, $ m-m' \in \operatorname{ker}g = \operatorname{im}f $, $ \exists l \in L $ such that $ f(l)=m-m' $. Hnece $ m-m'=f(l)= f(hf)(l)= f(h(m)-h(m'))=f(h(m))-f(h(m')) $. Hence $ m'-f(h(m'))=m-f(h(m)) $. 
