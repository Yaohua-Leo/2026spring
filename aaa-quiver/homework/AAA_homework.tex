...
\[
M \longrightarrow \bigoplus_{i\in Q_0} I(i)^{\dim (\operatorname{soc}M)_i}
\]
is an injective envelope of $M$. (I think here should be $\dim (\operatorname{soc}M)_i$ but not $\dim M_i$ to make sure that the embedding is essential injective.)

\begin{proof}
    Let
    \[
    m_i:=\dim_k(\operatorname{soc}M)_i
    \qquad\text{and}\qquad
    E:=\bigoplus_{i\in Q_0} I(i)^{m_i}.
    \]
    Since $\operatorname{soc}M$ is semisimple and the simple representations of $Q$ are
    precisely the $S(i)$, we have
    \[
    \operatorname{soc}M \cong \bigoplus_{i\in Q_0} S(i)^{m_i}.
    \]
    
    By Problem~17, applied to the simple representation $S(j)$, we get
    \[
    \operatorname{Hom}(S(j),I(i)) \cong S(j)_i=
    \begin{cases}
    k & \text{if } i=j,\\
    0 & \text{if } i\neq j.
    \end{cases}
    \]
    Thus for each $i$ there is, up to scalar, a unique nonzero morphism
    \[
    S(i)\longrightarrow I(i),
    \]
    and there is no nonzero morphism $S(j)\to I(i)$ for $j\neq i$. Therefore the above
    decomposition of $\operatorname{soc}M$ yields an injective morphism
    \[
    u\colon \operatorname{soc}M \hookrightarrow E
    \]
    whose image is exactly $\operatorname{soc}E$.
    
    Now let
    \[
    s\colon \operatorname{soc}M \hookrightarrow M
    \]
    be the inclusion. Since $E$ is injective (being a direct sum of injective representations),
    the morphism $u$ extends along $s$ to a morphism
    \[
    \iota\colon M \longrightarrow E
    \]
    such that
    \[
    \iota\circ s=u.
    ...
    We claim that $\iota$ is injective. Suppose $\ker\iota\neq 0$. Since $\ker\iota$ is a
    nonzero finite-dimensional representation, it contains a simple subrepresentation. Let $S\subseteq \ker\iota$ be simple. Then $S\subseteq M$ is a
    simple subrepresentation, hence
    \[
    S\subseteq \operatorname{soc}M.
    \]
    But $\iota|_{\operatorname{soc}M}=u$ is injective, so $\iota$ cannot vanish on the
    nonzero subrepresentation $S$, a contradiction. Hence
    \[
    \ker\iota=0,
    \]
    so $\iota$ is a monomorphism.
    
    Next we show that $\iota(M)$ is an essential subrepresentation of $E$. Let
    $0\neq X\subseteq E$ be any nonzero subrepresentation. Again,
    $X$ contains a simple subrepresentation $S$. Since $S$ is simple and $S\subseteq E$,
    we have
    \[
    S\subseteq \operatorname{soc}E=\operatorname{im}(u)\subseteq \iota(M).
    \]
    Therefore
    \[
    0\neq S\subseteq X\cap \iota(M),
    \]
    so $X\cap \iota(M)\neq 0$. Thus $\iota(M)$ is essential in $E$.
    
    Finally, let
    \[
    j\colon M \hookrightarrow J
    \]
    be any injective morphism into an injective representation $J$. Since $J$ is injective
    and $\iota\colon M\hookrightarrow E$ is a monomorphism, there exists a morphism
    \[
    h\colon E\longrightarrow J
    \]
    such that
    \[
    h\circ \iota=j.
    \]
    We show that $h$ is injective. If $\ker h\neq 0$, then since $\iota(M)$ is essential in
    $E$, we must have
    \[
    \ker h\cap \iota(M)\neq 0.
    \]
    But on $\iota(M)$ we have $h\circ \iota=j$, and $j$ is injective, so
    $h|_{\iota(M)}$ is injective. Hence
    \[
    \ker h\cap \iota(M)=0,
    \]
    a contradiction. Therefore $\ker h=0$, so $h$ is injective.
    
    Thus for every injective morphism $j\colon M\hookrightarrow J$ into an injective
    representation $J$, there exists an injective morphism $h\colon E\hookrightarrow J$
    such that
    \[
    h\circ \iota=j.
    \]
    Therefore $\iota\colon M\hookrightarrow E$ is an injective envelope of $M$.
    
    Hence the natural embedding
    \[
    M \longrightarrow \bigoplus_{i\in Q_0} I(i)^{\dim(\operatorname{soc}M)_i}
    \]
