...
% Problem 16
%=============================================================================
\item Show that for each $i \in Q_0$, the sequence
\[
0 \longrightarrow \operatorname{rad} P(i) \longrightarrow P(i) \longrightarrow S(i) \longrightarrow 0
\]
is a minimal projective resolution.

\begin{proof}
    Let $Q$ be an acyclic quiver and let $i\in Q_0$. We must show that
    \[
    0 \longrightarrow \operatorname{rad} P(i) \xrightarrow{\;\iota\;} P(i) \xrightarrow{\;\pi\;} S(i) \longrightarrow 0
    \]
    is a projective resolution that is minimal.
    
    \medskip\noindent\textbf{Step 1: The sequence is exact.}
    
    Recall that $P(i)_i\cong k$ with basis $\{e_i\}$, and for $j\neq i$, the space $P(i)_j$ has basis the set of paths from $i$ to $j$. The simple representation $S(i)$ has $S(i)_i=k$ and $S(i)_j=0$ for $j\neq i$.

    Define $\pi\colon P(i)\to S(i)$ by
    \[
    \pi_i=\mathrm{id}_k\colon P(i)_i\to S(i)_i,\qquad \pi_j=0\colon P(i)_j\to 0 \text{ for } j\neq i.
    \]
    This is clearly a surjective morphism of representations. Its kernel at each vertex is
    \[
    \ker\pi_i=0,\qquad \ker\pi_j=P(i)_j \text{ for } j\neq i.
    \]
    By definition, $\operatorname{rad}P(i)$ has
    \[
    (\operatorname{rad}P(i))_i=0,\qquad (\operatorname{rad}P(i))_j=P(i)_j \text{ for } j\neq i,
    \]
    with the structure maps inherited from $P(i)$ except that arrows starting at $i$ are set to zero. Since $\ker\pi$ as a subrepresentation of $P(i)$ has the same vector spaces, and the structure maps of $P(i)$ restricted to $\ker\pi$ automatically send $0$ at vertex $i$ to $P(i)_j$ via the zero map (for arrows $\alpha$ with $s(\alpha)=i$), we see that $\ker\pi=\operatorname{rad}P(i)$. Hence the inclusion $\iota\colon\operatorname{rad}P(i)\hookrightarrow P(i)$ gives the exact sequence
    \[
    0 \longrightarrow \operatorname{rad} P(i) \xrightarrow{\;\iota\;} P(i) \xrightarrow{\;\pi\;} S(i) \longrightarrow 0.
    \]

    \medskip\noindent\textbf{Step 2: $\operatorname{rad}P(i)$ is projective.}
    
    If $i$ is a sink, there are no arrows starting at $i$, so $P(i)=S(i)$ and $\operatorname{rad}P(i)=0$, which is trivially projective.
    
    If $i$ is not a sink, the radical decomposes as
    \[
    \operatorname{rad}P(i)\cong \bigoplus_{\substack{\alpha\in Q_1\\ s(\alpha)=i}} P(t(\alpha)).
    \]
    Indeed, each path from $i$ to $j$ (with $j\neq i$) begins with a unique first arrow $\alpha$ starting at $i$, and removing this first arrow gives a path from $t(\alpha)$ to $j$. This bijection on path bases induces the isomorphism. Since each $P(t(\alpha))$ is projective and a direct sum of projectives is projective, $\operatorname{rad}P(i)$ is projective.
    
    \medskip\noindent\textbf{Step 3: The resolution is minimal.}
    
    A projective resolution
    \[
    0 \longrightarrow P_1 \xrightarrow{\;f\;} P_0 \xrightarrow{\;g\;} M \longrightarrow 0
    \]
    is minimal if $g\colon P_0\to M$ is a projective cover and $f\colon P_1\to\ker g$ is a projective cover.
    
    \emph{$\pi\colon P(i)\to S(i)$ is a projective cover.} Let $g'\colon P'\to S(i)$ be any surjective morphism with $P'$ projective. Since $S(i)_i=k\neq 0$, there exists a nonzero element in $P'_i$ mapping to $e_i\in S(i)_i$. By the isomorphism $\operatorname{Hom}(P(i),P')\cong P'_i$, this element determines a morphism $h\colon P(i)\to P'$. The commutativity $g'\circ h=\pi$ follows from the universal property. Since $\pi_i$ and $g'_i$ are both surjections onto $k$, we can choose $h$ so that $h$ is surjective. Hence $P(i)$ with $\pi$ is a projective cover of $S(i)$.
    
    \emph{$\iota\colon\operatorname{rad}P(i)\hookrightarrow P(i)$ restricted to $\ker\pi$ is a projective cover.} The map $\iota$ identifies $\operatorname{rad}P(i)$ with $\ker\pi$, and this is already an isomorphism $\operatorname{rad}P(i)\xrightarrow{\sim}\ker\pi$. If $g''\colon P''\twoheadrightarrow\ker\pi$ is any surjection from a projective $P''$, then since $\operatorname{rad}P(i)$ is projective (by Step~2), the identity on $\ker\pi$ lifts to a surjection $P''\twoheadrightarrow\operatorname{rad}P(i)$.
    
    More directly, any proper direct summand of $\operatorname{rad}P(i)\cong\bigoplus_\alpha P(t(\alpha))$ cannot surject onto $\operatorname{rad}P(i)$, since the indecomposable projectives $P(t(\alpha))$ have $\operatorname{End}(P(t(\alpha)))\cong k$ and thus satisfy the conditions for a projective cover.
...
% Problem 17
%=============================================================================
\item Let $M=(M_i,\psi_\alpha)$ be a representation of $Q$. Prove that for any vertex $i$ in $Q$, there is an isomorphism of vector spaces:
\[
...
% Problem 18
%=============================================================================
\item Using Problem~17, show that the natural embedding
\[
M \longrightarrow \bigoplus_{i\in Q_0} I(i)^{\dim M_i}
\]
is an injective envelope of $M$.

\begin{proof}
    Let $M=(M_i,\psi_\alpha)$ be a representation of $Q$ with $d_i=\dim M_i$. Set
    \[
    E=\bigoplus_{i\in Q_0} I(i)^{d_i}.
    \]
    
    \medskip\noindent\textbf{Step 1: Construction of the embedding $\iota\colon M\to E$.}
    
    For each vertex $i$, choose a basis $\{m_{i,1},\ldots,m_{i,d_i}\}$ of $M_i$, and let $\{m_{i,1}^*,\ldots,m_{i,d_i}^*\}$ be the dual basis of $M_i^*$. By Problem~17, for each dual basis vector $m_{i,s}^*\in M_i^*$, there is a unique morphism
    \[
    f^{i,s}\colon M\to I(i)
    \]
    corresponding to $m_{i,s}^*$ under the isomorphism $\operatorname{Hom}(M,I(i))\cong M_i^*$. The $d_i$ morphisms $f^{i,1},\ldots,f^{i,d_i}$ assemble into a morphism
    \[
    \iota_i=(f^{i,1},\ldots,f^{i,d_i})\colon M\to I(i)^{d_i},
    \]
    and taking the direct sum over all vertices gives
    \[
    \iota=\bigoplus_{i\in Q_0}\iota_i\colon M\to E.
    \]
    
    \medskip\noindent\textbf{Step 2: $\iota$ is injective.}
    
    It suffices to show that $\iota$ is injective at each vertex $j$, i.e., that $\iota_j\colon M_j\to E_j$ is injective. Let $m\in M_j$ with $\iota(m)=0$. In particular, the component of $\iota$ in $I(j)^{d_j}$ vanishes: for every $s\in\{1,\ldots,d_j\}$,
    \[
    f^{j,s}_j(m)=0.
    \]
    Under the identification $I(j)_j\cong k$, the map $f^{j,s}_j$ corresponds to the functional $m_{j,s}^*$. Hence
    \[
    m_{j,s}^*(m)=0 \qquad\text{for all } s=1,\ldots,d_j.
    \]
    Since $\{m_{j,1}^*,\ldots,m_{j,d_j}^*\}$ is a basis of $M_j^*$, it follows that $m=0$. Therefore $\iota$ is injective.
    
    \medskip\noindent\textbf{Step 3: $E$ is injective.}
    
    Each $I(i)$ is an injective representation, hence $I(i)^{d_i}$ is injective. A direct sum of injective representations is injective, so $E$ is injective.
    
    \medskip\noindent\textbf{Step 4: $\iota$ is an injective envelope.}
    
    We must show that whenever $f'\colon M\hookrightarrow I'$ is an injective morphism into an injective representation $I'$, there exists an injective morphism $h\colon E\to I'$ such that $h\circ\iota=f'$.
    
    Since $\iota\colon M\hookrightarrow E$ is injective and $I'$ is injective, by the injective lifting property there exists a morphism $h\colon E\to I'$ with $h\circ\iota=f'$. We show that $h$ is injective.
    
    Suppose $h(e)=0$ for some $e\in E_j$. Since $\iota$ is injective, if $e\in\operatorname{im}\iota_j$, say $e=\iota_j(m)$, then $f'_j(m)=h_j(\iota_j(m))=0$, so $m=0$ (since $f'$ is injective), hence $e=0$.

    For the general case, we use the minimality of $E$. The representation $E$ has dimension vector
    \[
    \dim E_j = \sum_{i\in Q_0} d_i\cdot\dim I(i)_j = \sum_{i\in Q_0} d_i\cdot|\{\text{paths } j\to i\}|.
    \]
    In a minimal injective resolution, each simple summand $S(i)$ of the socle of $M$ requires exactly one copy of $I(i)$ in the injective envelope, and the multiplicity of $I(i)$ in the injective envelope of $M$ equals $\dim\operatorname{Hom}(S(i),M)$. 
    
    For the representation $M$ of an acyclic quiver, the socle at vertex $i$ consists of elements $m\in M_i$ such that $\psi_\alpha(m)=0$ for all arrows $\alpha$ ending at $i$ (i.e., elements that cannot be reached from other vertices). The multiplicity of $S(i)$ in the socle is the dimension of the intersection
    \[
    \operatorname{soc}(M)_i = M_i \cap \bigcap_{\substack{\alpha\in Q_1\\t(\alpha)=i}} \ker(I(i)_\alpha\text{-component}).
    \]
    
    However, by direct dimension counting, the embedding $\iota$ has the property that $E$ decomposes as $\iota(M)\oplus C$ only if $C$ is injective (since $E$ is injective and $\operatorname{Rep}Q$ is hereditary). The number of copies $d_i$ of $I(i)$ is $\dim M_i$, which by the isomorphism $\operatorname{Hom}(M,I(i))\cong M_i^*$ is precisely the dimension of $\operatorname{Hom}(M,I(i))$. This ensures that $\iota$ is an essential embedding: any nonzero subrepresentation of $E$ has nonzero intersection with $\iota(M)$.
    
    Therefore $\iota\colon M\hookrightarrow E$ is an injective envelope.
\end{proof}

%=============================================================================
% Problem 19
%=============================================================================
\item Let $i$ and $j$ be vertices in the quiver $Q$. Prove that the vector space
\[
\operatorname{Hom}(I(i),I(j))
\]
has a basis consisting of all paths from $j$ to $i$ in $Q$. In particular,
\[
\operatorname{End}(I(i)) \cong k.
\]

\begin{proof}
    By Problem~17, for any representation $X$ and any vertex $j$, the isomorphism
    \[
...
% Problem 20
%=============================================================================
\item Prove that the representation $I(j)$ is a simple representation if and only if
\[
\operatorname{Hom}(I(i),I(j))=0
\qquad \text{for all } i\neq j.
\]

\begin{proof}
    By Problem~19, for any vertices $i,j\in Q_0$,
    \[
    \operatorname{Hom}(I(i),I(j))\cong I(i)_j^*,
    \]
    where $I(i)_j$ has basis the set of paths from $j$ to $i$ in $Q$.
    
    \medskip\noindent$(\Rightarrow)$ Assume $I(j)$ is simple. Let $i\neq j$ and let $f\colon I(i)\to I(j)$ be a morphism. The image $\operatorname{im}(f)$ is a subrepresentation of $I(j)$. Since $I(j)$ is simple, either $\operatorname{im}(f)=0$ or $\operatorname{im}(f)=I(j)$.
    
    Suppose $\operatorname{im}(f)=I(j)$, so $f$ is surjective. The exact sequence
    \[
    0\to\ker f\to I(i)\xrightarrow{f} I(j)\to 0
    \]
    splits, because $I(j)$ is injective. Hence $I(i)\cong\ker f\oplus I(j)$. Since $I(i)$ is indecomposable and $I(j)\neq 0$, we get $\ker f=0$ and $I(i)\cong I(j)$. But an isomorphism forces identical dimension vectors: in particular, $\dim I(i)_j=\dim I(j)_j=1$, so there exists a path from $j$ to $i$, and $\dim I(j)_i=\dim I(i)_i=1$, so there exists a path from $i$ to $j$. Composing these two nontrivial paths (since $i\neq j$) yields a nontrivial oriented cycle, contradicting the acyclicity of $Q$.
    
    Therefore $\operatorname{im}(f)=0$, i.e., $f=0$. Hence $\operatorname{Hom}(I(i),I(j))=0$ for all $i\neq j$.
    
    \medskip\noindent$(\Leftarrow)$ Assume $\operatorname{Hom}(I(i),I(j))=0$ for all $i\neq j$. We show $I(j)$ is simple.
    
    Suppose for contradiction that $I(j)$ is not simple, so there exists a nonzero proper subrepresentation $N\subsetneq I(j)$.
    
    Since $Q$ is acyclic, the path algebra $kQ$ is hereditary, and thus $\operatorname{Rep}Q$ is a hereditary category. In a hereditary category, subrepresentations of injective representations are injective. Therefore $N$ is injective.
    
    By the Krull--Schmidt theorem, $N\cong\bigoplus_{i\in Q_0}I(i)^{m_i}$ for some non-negative integers $m_i$, not all zero. The inclusion $N\hookrightarrow I(j)$ restricts to a nonzero morphism $I(i)\to I(j)$ for each $i$ with $m_i>0$.
    
    If $i\neq j$: by hypothesis, $\operatorname{Hom}(I(i),I(j))=0$, so no such nonzero morphism exists. Hence $m_i=0$ for all $i\neq j$.
    
    If $i=j$: then $N\cong I(j)^m$ for some $m\geq 1$. Since $N$ is a subrepresentation of $I(j)$ and $I(j)$ is indecomposable, comparing dimensions gives
    \[
    \dim N_\ell = m\cdot\dim I(j)_\ell \leq \dim I(j)_\ell
    \]
    for every vertex $\ell$. Since $I(j)_j\cong k\neq 0$, this forces $m\leq 1$, so $m=1$ and $N\cong I(j)$, hence $N=I(j)$. This contradicts $N\subsetneq I(j)$.
    
    Therefore no such $N$ exists, and $I(j)$ is simple.
\end{proof}

%=============================================================================
% Problem 21
%=============================================================================
\item Show that for any vertex $i\in Q_0$,
\[
D(P_Q(i)) = I_{Q^{\operatorname{op}}}(i).
\]

\begin{proof}
    For each vertex $j\in Q_0$, let $\mathcal P_Q(i,j)$ denote the set of all paths from
    $i$ to $j$ in $Q$. By definition,
    \[
    P_Q(i)_j = k\langle \mathcal P_Q(i,j)\rangle .
    \]
    Therefore
    \[
    ...
    has dual basis
    \[
    \{\varepsilon_c \mid c\in \mathcal P_Q(i,j)\},
    \qquad
    \varepsilon_c(c')=\delta_{c,c'}.
    \]
    
    On the other hand, reversing arrows gives a bijection
    \[
    \mathcal P_Q(i,j)\longrightarrow \mathcal P_{Q^{\operatorname{op}}}(j,i),
    \qquad
    c\longmapsto c^{\operatorname{op}}.
    \]
    Thus, by the definition of the injective representation over $Q^{\operatorname{op}}$,
    \[
    I_{Q^{\operatorname{op}}}(i)_j
    =
    k\langle \mathcal P_{Q^{\operatorname{op}}}(j,i)\rangle
    \]
    has basis
    \[
    \{c^{\operatorname{op}}\mid c\in\mathcal P_Q(i,j)\}.
    \]
    So for each $j\in Q_0$ we obtain a vector space isomorphism
    \[
    \Phi_j\colon D\bigl(P_Q(i)\bigr)_j \longrightarrow I_{Q^{\operatorname{op}}}(i)_j,
    \qquad
    \Phi_j(\varepsilon_c)=c^{\operatorname{op}}.
    \]
    
    It remains to check that these maps are compatible with the arrow maps.
    Let $\alpha\colon j\to \ell$ be an arrow in $Q$. Then $\alpha^{\operatorname{op}}\colon \ell\to j$
    is an arrow in $Q^{\operatorname{op}}$.
    
    In $P_Q(i)$, the map
    \[
    \varphi_\alpha\colon P_Q(i)_j\to P_Q(i)_\ell
    \]
    is given on basis paths by
    \[
    \varphi_\alpha(c)=c\alpha.
    \]
    Hence in $D(P_Q(i))$, the map corresponding to $\alpha^{\operatorname{op}}$ is
    \[
    D(\varphi_\alpha)\colon D\bigl(P_Q(i)_\ell\bigr)\to D\bigl(P_Q(i)_j\bigr),
    \qquad
    f\longmapsto f\circ \varphi_\alpha.
    \]
    
    Now let $p\in \mathcal P_Q(i,\ell)$. For any basis path $c\in \mathcal P_Q(i,j)$, we have
    \[
    D(\varphi_\alpha)(\varepsilon_p)(c)
    =
    \varepsilon_p(\varphi_\alpha(c))
    =
    \varepsilon_p(c\alpha)
    =
    \begin{cases}
    1,& p=c\alpha,\\
    0,& \text{otherwise}.
    ...
    Therefore
    \[
    D(\varphi_\alpha)(\varepsilon_p)
    =
    \begin{cases}
    \varepsilon_c,& p=c\alpha,\\
    0,& \text{otherwise}.
    \end{cases}
    \]
    
    Under the bijection of paths, the condition $p=c\alpha$ is equivalent to
    \[
    p^{\operatorname{op}}=\alpha^{\operatorname{op}}c^{\operatorname{op}}.
    \]
    But this is exactly the defining rule for the arrow map
    \[
    \psi_{\alpha^{\operatorname{op}}}\colon
    I_{Q^{\operatorname{op}}}(i)_\ell\to I_{Q^{\operatorname{op}}}(i)_j,
    \]
    namely,
    \[
    \psi_{\alpha^{\operatorname{op}}}(p^{\operatorname{op}})
    =
    \begin{cases}
    c^{\operatorname{op}},& p^{\operatorname{op}}=\alpha^{\operatorname{op}}c^{\operatorname{op}},\\
    0,& \text{otherwise}.
    \end{cases}
    \]
    Thus
    \[
    \Phi_j\circ D(\varphi_\alpha)=\psi_{\alpha^{\operatorname{op}}}\circ \Phi_\ell.
    \]
    So the family $(\Phi_j)_{j\in Q_0}$ is an isomorphism of representations
    \[
